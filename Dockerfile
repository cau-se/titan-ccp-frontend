...
CMD cp /etc/nginx/nginx.conf /etc/nginx/nginx.tmpl.conf \
    && envsubst '$SENSOR_MANAGEMENT_BASE_URL $HISTORY_BASE_URL $STATS_BASE_URL $FORECAST_BASE_URL $ANOMALY_DETECTION_BASE_URL' < /etc/nginx/nginx.tmpl.conf > /etc/nginx/nginx.conf \
    && nginx -g "daemon off;"