...
class CanvasDataPlot {
  ...
  public constructor (parentElement: HTMLElement, canvasDimensions: Array<number>, config: any) {
    ...
    this.showTooltips = (config.showTooltips !== false)
  ...
  protected updateTooltip (): void {
    ...
    let hitMarker = false
    // eslint-disable-next-line no-labels, @typescript-eslint/camelcase
    ...
    for (let i = 0; i < nDataSets; ++i) {
      ...
      for (let j = iStart; j <= iEnd; ++j) {
        ...
        if (dx * dx + dy * dy <= this.tooltipRadiusSquared) {
          ...
          // eslint-disable-next-line @typescript-eslint/camelcase, no-labels
  ...
  private drawGrid (): void {
    ...
    this.yScale.ticks(this.yAxis.ticks()[0])
      .map((d: any) => { return Math.floor(this.yScale(d)) + 0.5 })
      .forEach((d: any) => {
        this.canvas.moveTo(0, d)
        this.canvas.lineTo(this.width, d)
      })
    this.xScale.ticks(this.xAxis.ticks()[0])
      .map((d: any) => { return Math.floor(this.xScale(d)) + 0.5 })
      .forEach((d: any) => {
